...
<p align="center">
  <a href="./README.en.md">English overview</a>
</p>
...
| GuardFilter.sys | 可选组件 | 需 WDK 编译 | minifilter 文件系统回调、通信端口、事件缓冲区 |
| GuardMonitor.sys | 可选组件 | 需 WDK 编译 | 进程/映像通知、ObRegisterCallbacks 进程保护、DeviceIoControl |
| guardian_ca.dll | 已完成 | guardian_ca.dll (24 KB) | WiX MSI 自定义动作：安装密钥验证、配置文件复制 |
| MSI 安装包 | 已完成 | WiX 源文件 | 统一安装包：密钥验证 + 配置文件选择 + 服务注册 |
| 一键卸载脚本 | 已完成 | scripts/uninstall.bat | 密钥验证 + 服务停止/删除 + 注册表/文件清理 |
| 测试套件 | 已完成 | GuardianTests.exe | `test/CMakeLists.txt` 注册 17 个测试源文件，使用 Google Test |
...
Layer 2: 可选内核监控 — GuardFilter.sys + GuardMonitor.sys（需 WDK）
Layer 3: 进程守护三角 — GuardianA + GuardianB + GuardianC
Layer 4: 紧急熔断机制 — 加密 → 擦除 → 删除 → 锁定
```
...
```
单事件 → LOG / ALERT / BLOCK / TERMINATE / ENCRYPT（由策略配置决定）
...
**关键原则**: 单事件不会通过配置触发 WIPE / LOCKDOWN；LOG、ALERT_USER、BLOCK、TERMINATE、ENCRYPT 仍由事件策略决定。BLOCK 为内核级 I/O 拦截（需 GuardFilter 驱动），驱动未加载时跳过（不降级为 TERMINATE）。
